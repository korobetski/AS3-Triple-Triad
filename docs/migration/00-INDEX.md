...
| Phase 4: UI Layer | 🔄 IN PROGRESS — 2026-08-02. **The game is playable end to end**: create a character and choose its collection, pick an opponent from that collection (filtered by the hour), play a match under the opponent's own rules against the Phase 3 AI, and the profile is written back. Six of the 32 screens exist — `MenuScreen`, `OptionsScreen`, `ProfileListScreen`, `ProfileCreateScreen`, `OpponentScreen`, `MatchScreen` — covering Tier 1 bar `HelpScreen`, the match half of Tier 2, and the one item of Tier 4 that needs no network. **The `ff8_` collection is reachable for the first time**: the AS3 hard-codes `MODE = 'ff14_'` and never changes it. Phase 3's debt is paid — the AI is wired and the UI suite reworked around an autonomous opponent. Not done: 26 screens, drag and drop, the theme system, the pre-match animations. Nothing is reviewed or approved. See [08-PHASE-4-UI-LAYER.md](./08-PHASE-4-UI-LAYER.md) § What was built | - | - | - |
