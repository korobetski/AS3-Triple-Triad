...
### 8. Socket.as - Network Communication
...
### 9. Save.as - Save/Load System

**Location**: `sources/src/tto/datas/Save.as`
...
**Key Properties**:
- `DATAS:Object` - Save data
- `SAVE_DIR:String` - Save directory

**Key Methods**:
- `setToDefaultValues():void` - Create new save with defaults
- `load(profile_name:String):Object` - Load save by name
- `loadFile(save:File):Object` - Load from file object
- `save():void` - Save current data
- `save(DATAS:Object):void` - Static save method
...
DATAS.DECKS = [{name:'Starter deck', cards:[1, 3, 6, 7, 10]}];
DATAS.STATS = { WINS:0, DEFEATS:0, DRAWS:0, FORFEITS:0 };
DATAS.BAG = [];
...
**Complexity**: **MEDIUM**
- ~98 lines
- Save file format: Encrypted JSON
- Uses CryptoHelper for encryption

**Migration Notes**:
- Encrypted JSON → SQLDelight or SharedPreferences + encryption
- Save format can remain similar (JSON)
- Use Kotlinx Serialization for JSON
- Platform-specific storage (SQLDelight for KMP)

---

### 10. Cards.as - Card Data

**Location**: `sources/src/tto/datas/cards.as`

**Purpose**:
- Contains all card definitions for both collections
...
**Complexity**: **MEDIUM**
- ~321 lines
- Static data, no logic
- Two separate collections

**Migration Notes**:
- Convert to JSON files
- Create data classes for Card
- Load from resources instead of hardcoded
- Localization keys remain the same
- Power values: hex strings ("4", "A", etc.)
...
### Tier 1: Foundation (Must be done first)
...
### Tier 2: Core Logic (Critical path)

1. **Rules Engine** (Phase 3)
   - tripleTriadRules
   - TTOCore
...
1. **Board System** (Phase 3)
   - Board
   - Tile
   - Card (display logic)

2. **Match Screens** (Phase 4)
   - BaseMatchScreen
   - PVEMatchScreen
   - PVPMatchScreen
...
3. **Screens** (Phase 4)
4. **Navigation** (Phase 4)

### Tier 5: Network

1. **WebSocket Client** (Phase 5)
2. **SocketManager** (Phase 5)
3. **Message Handling** (Phase 5)
...
1. **Start with Data Models**: Migrate Card, Tile, Board first
2. **Test Core Logic Early**: Validate TTOCore migration thoroughly
3. **Create PoC for UI**: Validate Compose can handle the animations
4. **Understand Network Protocol**: Reverse-engineer server messages
5. **Prioritize Testing**: Each migrated component needs tests
6. **Maintain Feature Parity**: Don't skip any features
7. **Optimize for Mobile**: Consider mobile constraints from start

---

## 📚 Next Steps

1. **Create migration plan** - DONE (this document)
2. **Set up project structure** - Phase 1
3. **Migrate data models** - Phase 2
4. **Migrate core logic** - Phase 3
5. **Migrate UI** - Phase 4

---

*For more details on specific components, see the detailed migration phase documents.*
